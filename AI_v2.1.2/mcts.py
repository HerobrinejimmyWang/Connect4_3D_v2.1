...
class MCTS:
    ...
    def get_action_prob(self, canonical_board, temp=0, valid_mask=None):
        """
        Runs simulations and returns probabilities.
        Optimized for inference: removed Dirichlet noise and training loops.
        """
        ...
        # 如果提供了有效动作掩码（用于限制搜索空间，如前期忽略高层）
        if valid_mask is not None:
            self.Ps[s] = self.Ps[s] * valid_mask
            sum_Ps_s = np.sum(self.Ps[s])
            if sum_Ps_s > 0:
                self.Ps[s] /= sum_Ps_s
            else:
                self.Ps[s] = self.game.get_valid_moves(canonical_board)
            self.Vs[s] = self.game.get_valid_moves(canonical_board) * valid_mask
